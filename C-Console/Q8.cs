...
namespace BattleShips_Exercise
{
    class Program
    {
        ...
        private static void PlayGame(ref char[,] Board, ref ShipType[] Ships)
        {
            ...
            {
                PrintBoard(Board);
                // call a function that returns true if the user wants save the game
                if (GetSavedGame() == true)
                {
                    SaveGame(Board); // save the game in a text file.
                    return;
                }
        ...
        private static void SaveGame(char[,] Board)
        {
            // look at SetUpBoard
            string line = "";
            StreamWriter saveFile = new StreamWriter(SavedGame);
            for (int Row = 0; Row < 10; Row++)
            {
                for (int Column = 0; Column < 10; Column++)
                {
